...
class QuestionAnswerScreen extends React.Component<Props, State> {
  ...
  onNext = () => {
    ...
    this.props.updateQuestionAnswer({
      ...
      answerId: this.props.questionsIsPlaying[this.state.index].answers[
        this.state.answerIsClicked
      ].id
    });
  };
    ...
    this.setState(
      {
        ...
        isOutOfQuestion:
          this.state.index + 1 === this.props.questionsIsPlaying.length
            ? true
            : false
      },
      () =>
        Animated.timing(this.state.answerAnimationValue, {
          toValue: 1,
          duration: 200,
      ...
      >
        <View style={styles.Container}>
          ...
          >
            <FastImage
              source={{
                uri: `http://localhost:3003${questionIsPlaying.coverUrl}`
              }}
              style={styles.Image}
              resizeMode="contain"
            />
          ...
          <View style={{ flex: 1.5, marginTop: 10, paddingHorizontal: "3%" }}>
            ...
            >
              <TextMontserrat style={styles.HeaderText}>
                {questionIsPlaying.description}
              </TextMontserrat>
            </View>
          ...
          >
            {this.state.isOutOfQuestion ? (
              ...
              >
                <TextMontserrat
                  style={{ color: "#fff", fontFamily: "Montserrat-ExtraBold" }}
                >
                  Finish
                </TextMontserrat>
              </TouchableOpacity>
              ...
              >
                <TouchableOpacity
                  style={[
                    ...
                    {
                      marginBottom: 12,
                      opacity: this.state.isDoneAnswering ? 1 : 0.6
                    }
                  ]}
                  onPress={this.onNext}
                  disabled={!this.state.isDoneAnswering}
                >
                  <TextMontserrat
                    style={{
                      color: "#fff",
                ...
                <View style={styles.LineContainer}>
                  <View
                    style={[
                      styles.Line,
                      {
                        width: `${((this.state.index + 1) /
                          this.props.questionsIsPlaying.length) *
                          100}%`
                      }
                    ]}
                  />
...
const mapDispatch = ({ navigation, quizzes }: RematchDispatch<models>) => ({
  ...
  updateQuestionAnswer: (value: QuestionAnswer) => {
    quizzes.updateQuestionAnswer(value as any);
  },
  submitAnswer: () => {
    quizzes.submitAnswer("" as any);
  }
});

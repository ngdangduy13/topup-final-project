...
import {
  ...
  Answer,
  UpdateAnswerParams,
  UpdateIsCorrectParams,
  SingleQuiz,
  QuestionAnswer,
  UpdateQuizToCreateParam,
  QuestionResult,
  QuestionChecklist,
  UpdateQuizParams
} from "./interface";
...
const navigation = createModel({
  ...
  reducers: {
    ...
    updateEndOfList: (
      state: QuizzesState,
      payload: { state: string; value: boolean }
    ) => {
      const newEndOfList = {
        PUBLISHED:
          payload.state === config().quizState.published
            ? payload.value
            : state.endOfList.PUBLISHED,
        DRAFT:
          payload.state === config().quizState.draft
            ? payload.value
            : state.endOfList.DRAFT,
        REMOVED:
          payload.state === config().quizState.removed
            ? payload.value
            : state.endOfList.REMOVED,
        SEARCHED:
          payload.state === config().quizState.searched
            ? payload.value
            : state.endOfList.SEARCHED
      };
      return {
        ...state,
    ...
    updateIsHavingQuestion: (
      state: QuizzesState,
      payload: { index: number; value: boolean }
    ) => {
      const newQuestionChecklist = Object.assign({}, state.questionChecklist);
      newQuestionChecklist.isHavingQuestion.splice(
        payload.index,
        1,
        payload.value
      );
    ...
    updateQuestionResult: (
      state: QuizzesState,
      payload: { value: SingleQuiz[]; isLoadMore: boolean }
    ) => {
      return {
        ...state,
    ...
    getQuizzesBeaconSuccess: (
      state: QuizzesState,
      payload: { value: SingleQuiz[]; isLoadMore: boolean }
    ) => {
      return {
        ...state,
        allQuizzesBeacon: payload.isLoadMore
          ? [...state.allQuizzesPublished, ...payload.value]
          : payload.value
      };
    },
    getQuizzesPublishedSuccess: (
      state: QuizzesState,
      payload: { value: SingleQuiz[]; isLoadMore: boolean }
    ) => {
      return {
        ...state,
        allQuizzesPublished: payload.isLoadMore
          ? [...state.allQuizzesPublished, ...payload.value]
          : payload.value
      };
    },
    getQuizzesDraftSuccess: (
      state: QuizzesState,
      payload: { value: SingleQuiz[]; isLoadMore: boolean }
    ) => {
      return {
        ...state,
        allQuizzesDraft: payload.isLoadMore
          ? [...state.allQuizzesDraft, ...payload.value]
          : payload.value
      };
    },
    getQuizzesRemovedSuccess: (
      state: QuizzesState,
      payload: { value: SingleQuiz[]; isLoadMore: boolean }
    ) => {
      return {
        ...state,
        allQuizzesRemoved: payload.isLoadMore
          ? [...state.allQuizzesRemoved, ...payload.value]
          : payload.value
      };
    },
    getQuizzesSearchedSuccess: (
      state: QuizzesState,
      payload: { value: SingleQuiz[]; isLoadMore: boolean }
    ) => {
      return {
        ...state,
        allQuizzesSearched: payload.isLoadMore
          ? [...state.allQuizzesSearched, ...payload.value]
          : payload.value
      };
    },
    deleteQuizSuccess: (
      state: QuizzesState,
      payload: { index: number; state: string }
    ) => {
      const newAllQuizzes =
        payload.state === config().quizState.published
      ...
      return {
        ...
        allQuizzesPublished:
          payload.state === config().quizState.published
            ? newAllQuizzes
            : state.allQuizzesPublished,
        allQuizzesDraft:
          payload.state === config().quizState.draft
            ? newAllQuizzes
            : state.allQuizzesDraft
      };
    },
    ...
    deleteQuestionToCreate: (state: QuizzesState, payload: number) => {
      ...
      const newQuestionChecklist: any = Object.assign(
        {},
        state.questionChecklist
      );
      Object.keys(newQuestionChecklist).map((item: any) =>
        newQuestionChecklist[item].splice(payload, 1)
      );
      return {
        ...state,
    ...
    addNewQuestion: (state: QuizzesState, payload: QuizzesState) => {
      ...
      return {
        ...state,
        questionToCreate: [
          ...state.questionToCreate,
          { ...newQuestion, id: state.questionToCreate.length }
        ],
        questionChecklist: {
          isHavingCoverUrl: [
            ...state.questionChecklist.isHavingCoverUrl,
            false
          ],
    ...
    updateQuizToCreate: (
      state: QuizzesState,
      payload: UpdateQuizToCreateParam
    ) => {
      const newQuizToCreate = {
        ...
        description: payload.description
          ? payload.description
          : state.quizToCreate.description,
        state: payload.state ? payload.state : state.quizToCreate.state,
        coverImageUrl: payload.coverImageUrl
          ? payload.coverImageUrl
          : state.quizToCreate.coverImageUrl,
        beacon: {
          isActive:
            payload.isActive !== undefined
              ? payload.isActive
              : state.quizToCreate.beacon.isActive,
          minor: payload.minor
            ? payload.minor
            : state.quizToCreate.beacon.minor,
          major: payload.major
            ? payload.major
            : state.quizToCreate.beacon.major,
    ...
    updateCoverUrlQuestion: (
      state: QuizzesState,
      payload: { coverUrl: string; index: number }
    ) => {
      const newQuestion: Question = {
        ...Object.assign({}, state.questionToCreate[payload.index]),
    ...
    updateAnswer: (state: QuizzesState, payload: UpdateAnswerParams) => {
      const newAnswer: Answer = {
        ...Object.assign(
          {},
          state.questionToCreate[payload.indexQuestion].answers[
            payload.indexAnswer
          ]
        ),
      ...
      const newAnswers = state.questionToCreate[
    ...
    updateIsCorrect: (state: QuizzesState, payload: UpdateIsCorrectParams) => {
      const newAnswer: Answer = {
        ...
        isCorrect: state.questionToCreate[payload.indexQuestion].answers[
          payload.indexAnswer
        ].isCorrect
          ? false
          : true
  ...
  effects: dispatch => ({
    async getAllQuizzes(
      payload: { state: string; isLoadMore: boolean },
      rootState: AppState
    ): Promise<any> {
      try {
        !payload.isLoadMore
        ...
        if (payload.state === config().quizState.published) {
          this.getQuizzesPublishedSuccess({
            value: data,
            isLoadMore: payload.isLoadMore
          });
        } else if (payload.state === config().quizState.draft) {
          this.getQuizzesDraftSuccess({
            value: data,
            isLoadMore: payload.isLoadMore
          });
        } else if (payload.state === config().quizState.removed) {
          this.getQuizzesRemovedSuccess({
            value: data,
            isLoadMore: payload.isLoadMore
          });
        }
    ...
    async getQuizzesBeacon(
      payload: { uuid: string; minor: number; major: number },
      rootState: AppState
    ): Promise<any> {
      try {
        // this.updateBusyState(true);
        ...
        const data = result.data.map((item: any, index: number) => {
          return {
            ...item,
            coverImageUrl: `${config().hostUrl}${config().imageUrl}${
              item.coverImageUrl
            }`
          };
        });
    ...
    async findQuizzes(
      payload: { value: string; isLoadMore: boolean; state: string },
      rootState: AppState
    ): Promise<any> {
      try {
        // this.updateBusyState(true);
        ...
        if (data.length === 51) {
          ...
          this.updateEndOfList({
            state: config().quizState.searched,
            value: true
          });
        }
        this.getQuizzesSearchedSuccess({
          value: data,
          isLoadMore: payload.isLoadMore
        });
      } catch (error) {
    ...
    async createQuiz(
      ...
      try {
        ...
        const result = await serviceProvider
          .ApiService()
          .createQuiz(rootState.userProfile.token, body);
        this.addQuiz(result);
    ...
    async getQuizById(
      payload: { id: string; isForCreate: boolean },
      rootState: AppState
    ): Promise<any> {
      try {
        this.updateBusyState(true);
        const result: any = await serviceProvider
          .ApiService()
          .findQuizz(rootState.userProfile.token, payload.id);
        if (payload.isForCreate) {
          ...
          const questionChecklist: QuestionChecklist = {
            isHavingCoverUrl: result.questions.map((item: any) =>
              item.coverUrl === "" ? false : true
            ),
            isHavingQuestion: result.questions.map((item: any) =>
    ...
    async deleteQuiz(
      payload: { id: string; index: number; state: string },
      rootState: AppState
    ): Promise<any> {
      try {
        this.updateBusyState(true);
        const result: any = await serviceProvider
          .ApiService()
          .deleteQuiz(rootState.userProfile.token, payload.id);
    ...
    async submitAnswer(payload: string, rootState: AppState): Promise<any> {
      try {
        ...
        const fetchAnswer: any = await fetch(
          "http://localhost:3003/api/quizzes/submitanswers",
          {
            method: "POST",
            headers: {
        ...
        const result = await fetchAnswer.json();
        // console.log(result);
        dispatch.userProfile.updateRewardPoint(result.rewardPoints);
    ...
    async activateQuiz(payload: string, rootState: AppState): Promise<any> {
      try {
        ...
        const result: any = await serviceProvider
          .ApiService()
          .activateQuiz(rootState.userProfile.token, payload);
        Toast.show({
    ...
    async uploadImage(
      payload: { uri: string; isQuestionCoverUrl: boolean; index?: number },
      rootState: AppState
    ): Promise<any> {
      try {
        ...
        const result: any = await fetch(
          "https://lumileds.techkids.io/api/uploadImage",
          {
            method: "POST",
            headers: {

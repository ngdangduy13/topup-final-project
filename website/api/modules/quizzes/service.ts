import { IFindQuizzQuery, IFindQuizzResult, ICreateQuizzInput, IFindQuizzDetail, IDeactivateQuizz, IUpdateQuizzInput, ISubmitAnswersInput, ISubmitAnswersResult, IActivateQuizz } from './interface';
...
const submitAnswer = async (body: ISubmitAnswersInput, user: any): Promise<ISubmitAnswersResult> => {
    ...
    for (const question of quizDetail.questions) {
        for (const answer of question.answers) {
            for (const userAnswer of body.answers) {
                if (userAnswer.questionId === question.id && answer.id === userAnswer.answerId && answer.isCorrect) {
    ...
    const scoreboard: any = { userId: body.userId, quizId: body.quizId, points, username: usernameResult, firstSubmittedAt: new Date() };
    // if (isUserScoreboardExist) {
    //     await quizScoreboardRepository.updateScoreboard(addModificationAuditInfo(user, scoreboard));
    // } else {
    // }
    const findScoreBoardByQuizIdAndUserId =
        await totalScoreBoardRepository.findScoreboard({
            userId: body.userId,
            quizId: body.quizId,
            pageIndex: '',
    ...
    if (!isUserScoreboardExist) {
        await totalScoreBoardRepository.createScoreboard(addCreationAuditInfo(user, {
            userId: body.userId,
            points,
        ...
        if (!isUserHavePlayedBefore) {
            ...
            await totalScoreBoardRepository.updateScoreboard(addModificationAuditInfo(user, {
                userId: body.userId,
                points,
                username: usernameResult,
                quizId: body.quizId,
    ...
    return { points, correctAnswerCount };
};

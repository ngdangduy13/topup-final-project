...
quizzesRouter.post(
  ...
  async (req: any, res) => {
    try {
      ...
      const results = await quizzService.createQuizz(
...
quizzesRouter.delete(
  ...
  async (req: any, res) => {
    try {
      await quizzService.deactivateQuizz(
        addModificationAuditInfo(req.query.profile, { id: req.params.id })
      );
...
quizzesRouter.get(
  ...
  async (req: any, res) => {
    try {
      await quizzService.activateQuizz(
        addModificationAuditInfo(req.query.profile, { id: req.params.id })
      );
      res.status(200).send(config.responseMessages.Success);
    } catch (error) {

import { IFindQuizzQuery, IFindQuizzResult, ICreateQuizzInput, IFindQuizzDetail, IDeactivateQuizz, IUpdateQuizzInput, IActivateQuizz } from './interface';
...
const deactivateQuizz = async (params: IDeactivateQuizz): Promise<void> => {
    try {
        const result = await QuizzModel.findOneAndUpdate(
            { _id: params.id },
            { $set: { state: 'REMOVED', lastModifiedAt: params.lastModifiedAt, lastModifiedBy: params.lastModifiedBy } },
            { new: true },
        ).exec();
        if (!result) {
...
const updateQuizz = async (body: IUpdateQuizzInput): Promise<void> => {
    try {
        body.questionCount = Object.keys(body.questions).length;
        await QuizzModel.findOneAndUpdate(
            { _id: body._id },
            { $set: body },
            { new: true },
        ).exec();
...
const findQuizzBydId = async (id: string): Promise<IFindQuizzDetail> => {
    try {
        return await QuizzModel.findOne({ _id: id })
            .exec() as IFindQuizzDetail;
    } catch (error) {
        logger.error(`${error.message} ${error.stack}`);
        throw new Error('Internal Server Error');
...
const activateQuizz = async (params: IActivateQuizz): Promise<void> => {
    try {
        const result = await QuizzModel.findOneAndUpdate(
            { _id: params.id },
            { $set: { state: 'DRAFT', lastModifiedAt: params.lastModifiedAt, lastModifiedBy: params.lastModifiedBy } },
            { new: true },
        ).exec();
        if (!result) {

...
class UserProfile extends React.Component<UserProfileProps, any> {
  ...
  _redeemPoint() {
    ...
    this.props.redeemPointEffect({
      points: parseInt(this.props.userPage.inputScorePoint),
      reason: this.props.userPage.inputReason,
      userId: (this.props.userPage.currentUser as any)._id,
    });
  }

  _updateUserRoles() {
    this.props.updateUserRoleEffect({
      id: (this.props.userPage.currentUser as any)._id,
      roles: (this.props.userPage.currentUser as any).roles,
    });
  }
  componentDidMount() {
      ...
      <div>
        ...
        <div className={'user-profile-page'}>
          <div className={'profile-wrapper'}>
            ...
            <Row className={'row'} gutter={16}>
              ...
              <Col span={12}>
                <Input
                  className={'input'}
                  disabled={true}
                  value={(this.props.userPage.currentUser as any)._id}
                />
              </Col>
              <Col span={8} />
            </Row>
            <Row className={'row'} gutter={16}>
              ...
              <Col span={12}>
                <Input
                  className={'input'}
                  disabled={false}
                  value={(this.props.userPage.currentUser as any).username}
                />
              </Col>
              <Col span={8} />
            </Row>
            <Row className={'row'} gutter={16}>
              ...
              <Col span={12}>
                <Input
                  className={'input'}
                  disabled={false}
                  value={(this.props.userPage.currentUser as any).email}
                />
              </Col>
              <Col span={8} />
            </Row>
            <Row className={'row'} gutter={16}>
              ...
              <Col span={12}>
                <Checkbox
                  disabled={
                    (this.props.userPage.currentUser as any)._id.toString() ===
                    this.props.userProfileData._id.toString()
                  }
                  onChange={() => this._setUserRoles(config.roles.admin)}
                  checked={
                    (this.props.userPage.currentUser as any).roles.filter(
                      (role: string) => role === config.roles.admin
                    ).length > 0
                  }
                >
                  Admin
                ...
                <Checkbox
                  ...
                  checked={
                    (this.props.userPage.currentUser as any).roles.filter(
                      (role: string) => role === config.roles.quizzMaster
                    ).length > 0
                  }
                >
                  Quizz Master
          ...
          <div className={'point-wrapper'}>
            <h1>Points</h1>
            <Row className={'row'} gutter={16}>
              <Col span={4}>Score Point</Col>
              <Col span={12}>
                <Input
                  className={'input'}
                  disabled={true}
                  value={(this.props.userPage.currentUser as any).scorePoint}
                />
              </Col>
              <Col span={8} />
            </Row>
            <Row className={'row'} style={{ marginBottom: '30px' }} gutter={16}>
              <Col span={4}>Reward Point</Col>
              <Col span={12}>
                <Input
                  className={'input'}
                  disabled={true}
                  value={(this.props.userPage.currentUser as any).rewardPoint}
                />
              </Col>
              <Col span={8} />
            </Row>
            <b>History exchange reward points</b>
            ...
            <Row className={'row'} gutter={16}>
              ...
              <Col span={12}>
                <Input
                  className={'input'}
                  value={this.props.userPage.inputReason}
                  onChange={e => this._onInputReasonChange(e.target.value)}
                />
              </Col>
              <Col span={8} />
            </Row>
            <Row className={'row'} gutter={16}>
              <Col span={4} />
              <Col span={12}>
                <Button onClick={() => this._redeemPoint()}>Redeem</Button>
              </Col>
              <Col span={8} />
            </Row> */}
            <UserPointList

...
class UserList extends React.Component<UserListProps, any> {
  ...
  _renderUserProfile(_item: IFindUserDetail) {
    ...
    this.props.fetchUserDetail(
      this.props.userPage.listUser.data.filter(
        user => user._id.toString() === _item._id.toString()
      )[0]
    );
  }
        ...
        {item.roles.map(
          (role: any) =>
            `${role === config.roles.admin ? 'Admin' : 'Quizz Master'}${
              item.roles.indexOf(role) === item.roles.length - 1 ? '' : ', '
            }`
        )}
      </span>
    );
    ...
    return [
      ...
      },
      {
        ...
        render: (_text: any, item: any, _index: any) => (
          <span>{item.scorePoint.toLocaleString()}</span>
        ),
      },
      {
        title: 'Reward Point',
        ...
        className: 'numberInput',
        render: (_text: any, item: any, _index: any) => (
          <span>{item.rewardPoint.toLocaleString()}</span>
        ),
      },
      {
        title: 'Created At',
        ...
        key: 'createdAt',
        render: (_text: any, item: any, _index: any) => (
          <span>{moment(item.createdAt).format('YYYY-MM-DD, h:mm:ss a')}</span>
        ),
      },
    ];
  }
  _onPageChange(isNext = true) {
    this.props.setPageIndex({
      userListPageIndex: isNext
        ? this.props.userPage.listUser.data[
            this.props.userPage.listUser.data.length - 1
          ].createdAt.toString()
      ...
      <div>
        ...
        <div style={{ textAlign: 'right', marginTop: '10px' }}>
          ...
          <Button
            ...
            disabled={this.props.userPage.isUserListNextDisabled}
          >
            {'>'}
          </Button>
        </div>

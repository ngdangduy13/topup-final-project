  ...
  return async (req: any, res: any, next: any) => {
    ...
    if (
      ...
      (tokenData.roles &&
        requiredPermission &&
        requiredPermission.length > 0 &&
        tokenData.roles.filter(
          (role: string) =>
            requiredPermission.filter(
              (requiredPermit: string) => role.trim() === requiredPermit.trim()
            ).length > 0
        ).length > 0)
    ) {
      req.query.profile = {
      ...
      res.cookie(`token`, token, {
        ...
        maxAge:
          tokenData.exp! - tokenData.iat! >= config.auth.expiresIn_7days
            ? config.auth.expiresIn_7days * 1000
            : config.auth.expiresIn_1day * 1000,
      });
    } else if (
      !tokenData.roles ||
      tokenData.roles.filter(
